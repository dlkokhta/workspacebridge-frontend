...
export const FilesTable = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return files.filter((f) => {
      if (q && !f.name.toLowerCase().includes(q) && !f.workspace.name.toLowerCase().includes(q)) return false;
      if (statusFilter === "active" && f.deletedAt) return false;
      if (statusFilter === "trashed" && !f.deletedAt) return false;
      return true;
    });
    ...
    <>
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <StatCard icon={<FileText size={14} />} label="Total files" value={stats.totalFiles} accent="green" />
          <StatCard icon={<HardDrive size={14} />} label={`Total storage — ${formatSize(stats.totalSize)}`} value={stats.perWorkspace.length} accent="blue" />
        </div>
      )}
      ...
      <div className="rounded-xl border border-black/[0.08] dark:border-white/[0.07] bg-white dark:bg-[#151a17] overflow-hidden">
        <div className="px-5 py-4 border-b border-black/[0.06] dark:border-white/[0.05]">
          <h3 className="text-[14px] font-semibold text-[#1a201c] dark:text-[#e8ece9]">Files</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-black/[0.06] dark:border-white/[0.05]">
                {["Name", "Workspace", "Uploaded by", "Type", "Size", "Status", "Created"].map((h) => (
            ...
            <tbody>
              {filtered.map((file) => (
                <tr key={file.id} className="border-b border-black/[0.04] dark:border-white/[0.04] last:border-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-3 text-[12px] text-[#858c87] dark:text-[#6e7672]">{file.mimeType}</td>
                  <td className="px-5 py-3 text-[12px] text-[#858c87] dark:text-[#6e7672]">{formatSize(file.size)}</td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center h-[20px] px-2 rounded-full border text-[10px] font-medium ${file.deletedAt ? "bg-[#c25a4a]/10 text-[#c25a4a] dark:text-[#e07b6b] border-[#c25a4a]/30" : "bg-[#5a8a6b]/10 text-[#3e6a4d] dark:text-[#6db383] border-[#5a8a6b]/30"}`}>
                      {file.deletedAt ? "Trashed" : "Active"}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-[12px] text-[#858c87] dark:text-[#6e7672]">{fmt(file.createdAt)}</td>
                  <td className="px-5 py-3">
                    <button onClick={() => deleteFile(file.id)} disabled={deletingId === file.id} className="w-7 h-7 flex items-center justify-center rounded-lg text-[#858c87] dark:text-[#6e7672] hover:bg-[#c25a4a]/10 hover:text-[#c25a4a] dark:hover:text-[#e07b6b] transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" title="Delete file permanently">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
