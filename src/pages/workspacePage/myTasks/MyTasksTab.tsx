...
import { usePrivateTasks } from "./usePrivateTasks";
...
export const MyTasksTab = ({ workspaceId }: MyTasksTabProps) => {
  const { tasks, loading, error, addTask, toggleTask, removeTask, clearError } =
    usePrivateTasks(workspaceId);
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden min-h-0">
      <div className="flex items-center justify-between px-6 py-3 border-b border-black/[0.06] dark:border-white/[0.05]">
        <span className="flex items-center gap-1.5 text-[13px] text-[#5a625e] dark:text-[#a0a8a3]">
          <Lock size={12} />
          {loading
            ? "Loading…"
            : `${openCount} open · ${doneCount} done · only visible to you`}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#fafaf7] dark:bg-[#0e1310]">
        ...
        >
          ...
          <input
            ...
            onKeyDown={handleKey}
            placeholder="Add a private note and press Enter…"
            maxLength={200}
            disabled={submitting}
          ...
          <div className="flex flex-col items-center justify-center gap-3 text-center py-16">
            ...
            <div>
              ...
              <p className="text-[13px] text-[#5a625e] dark:text-[#a0a8a3] max-w-xs">
                Personal notes and reminders about this client. Only you can
                see them — never visible to the client.
              </p>
            </div>
          </div>
        ) : (
